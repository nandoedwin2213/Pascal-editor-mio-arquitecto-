...
const wallModeConfig = {
  up: {
    icon: (props: any) => (
      <img alt={t('Full height')} height={28} src="/icons/room.webp" width={28} {...props} />
    ),
    label: 'Full height',
  },
  cutaway: {
    icon: (props: any) => (
      <img alt={t('Cutaway')} height={28} src="/icons/wallcut.webp" width={28} {...props} />
    ),
    label: 'Cutaway',
  },
  down: {
    icon: (props: any) => (
      <img alt={t('Low')} height={28} src="/icons/walllow.webp" width={28} {...props} />
    ),
    label: 'Low',
  },
}
      ...
      <DropdownMenuContent align="center" className="min-w-56" side="top">
        ...
        >
          ...
          <span className="ml-auto text-muted-foreground text-xs">
            {shadows ? t('On', 'shadows') : t('Off', 'shadows')}
          </span>
        </DropdownMenuItem>
        <DropdownMenuItem
          onSelect={(e) =>
        ...
        >
          ...
          <span>{t('Colors')}</span>
          <span className="ml-auto text-muted-foreground text-xs">
            {textures ? t('Colored') : t('Monochrome')}
          </span>
        </DropdownMenuItem>
        ...
        <DropdownMenuSub>
          ...
          <DropdownMenuSubContent className="min-w-56">
            {EDGE_OPTIONS.map((option) => (
              ...
              >
                <div className="flex flex-col">
                  <span className="text-foreground">{t(option.name, 'edges')}</span>
                  <span className="text-muted-foreground text-xs">{t(option.detail)}</span>
                </div>
      ...
      <TooltipProvider delayDuration={0}>
        <div className="corner-smooth pointer-events-auto flex h-12 max-w-[calc(100vw-1rem)] flex-row items-center justify-center gap-0.5 overflow-hidden rounded-2xl border border-border/40 bg-background/95 p-1 shadow-elevation-4 backdrop-blur-xl transition-colors duration-200 ease-out sm:h-14 sm:gap-1.5 sm:p-1.5">
          ...
          <ActionButton
            ...
            label={`${t('Levels')}: ${levelMode === 'manual' ? t('Manual') : t(levelModeLabels[levelMode as keyof typeof levelModeLabels] ?? 'Stacked', 'levels')}`}
            onClick={() => {
          ...
          {!glbActive && (
            <ActionButton
              ...
              label={`${t('Walls')}: ${t(wallModeConfig[safeWallMode].label, 'walls')}`}
              onClick={() => {
          ...
          <ActionButton
            ...
            label={`Recorrido: ${walkthroughActive ? t('On') : 'inactivo'}`}
