    ...
    <div className="relative border-border/50 border-b">
      ...
      <div className="relative flex items-center justify-between py-2 pr-3 pl-10">
        ...
        <div className="flex items-center gap-2">
          ...
          <span className="font-medium text-sm">Línea de lindero</span>
        </div>
        <button
          className={cn(
    ...
    <Popover onOpenChange={onOpenChange} open={open}>
      ...
      >
        <div className="flex flex-col gap-0.5">
          ...
          >
            <Camera className="h-3.5 w-3.5" />
            {hasCamera ? 'Actualizar vista' : 'Guardar vista'}
          </button>
          {hasCamera && (
            <button
    ...
    <div className="relative flex flex-col">
      ...
      >
        ...
        >
          ...
          <InlineRenameInput
            defaultName={isCapture ? 'Captura' : 'Imagen guía'}
            isEditing={isEditing}
            nodeId={refNode.id}
...
const LevelReferences = memo(function LevelReferences({
  ...
  const handleAddAsset = async (e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    if (!(isScan || isImage)) {
      ...
      useUploadStore
        .getState()
        .setError(levelId, 'Tipo de archivo no válido. Sube un escaneo .glb/.gltf o una imagen.')
      return
    }
    ...
    if (file.size > MAX_FILE_SIZE) {
      ...
      useUploadStore
        ...
        .setError(
          levelId,
          `El archivo es demasiado grande (${(file.size / 1024 / 1024).toFixed(0)} MB). El máximo son 200 MB.`,
        )
      return
    }
    ...
    if (isImage) {
      ...
      try {
        ...
        useUploadStore.getState().setError(levelId, 'No se pudo agregar esa imagen guía.')
      }
      return
    }
    ...
    if (!projectId) {
      ...
      useUploadStore.getState().setError(levelId, 'No hay un proyecto activo. Abre uno primero.')
      return
    }
          ...
          return (
            <div className="group/ref relative border-border/50 border-b" key="upload">
              ...
              >
                ...
                {uploading ? `Subiendo… ${progress}%` : 'Subir escaneo o plano'}
              </button>
        ...
        <Popover onOpenChange={setCameraPopoverOpen} open={cameraPopoverOpen}>
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              >
                <Camera className="h-3.5 w-3.5" />
                {level.camera ? 'Actualizar vista' : 'Guardar vista'}
              </button>
              {level.camera && (
                <button
                  className="flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-popover-foreground text-sm hover:bg-destructive hover:text-destructive-foreground"
        ...
        <Popover>
          ...
          <PopoverContent align="start" className="w-48 p-1" side="right">
            ...
            <button
              ...
              title="Duplicar nivel con opciones"
            >
              <Copy className="h-3.5 w-3.5" />
            ...
            <button
              ...
              title={canDeleteLevel ? 'Eliminar nivel' : 'La planta baja no se puede eliminar'}
            >
              <Trash2 className="h-3.5 w-3.5" />
              Eliminar
            </button>
        ...
        <Popover onOpenChange={setCameraPopoverOpen} open={cameraPopoverOpen}>
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              >
                <Camera className="h-3.5 w-3.5" />
                {zone.camera ? 'Actualizar vista' : 'Guardar vista'}
              </button>
              {zone.camera && (
                <button
                  className="flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-popover-foreground text-sm hover:bg-destructive hover:text-destructive-foreground"
    ...
    >
      ...
      >
        <div className="flex h-full min-w-0 flex-1 cursor-pointer items-center gap-2 py-2 pl-3">
          ...
          />
          <span className="truncate font-medium text-sm">{building.name || 'Edificación'}</span>
        </div>
        <Popover
        ...
        >
          ...
          >
            <div className="flex flex-col gap-0.5">
              ...
              >
                <Camera className="h-3.5 w-3.5" />
                {building.camera ? 'Actualizar vista' : 'Guardar vista'}
              </button>
              {building.camera && (
                <button
                  className="flex w-full cursor-pointer items-center gap-2 rounded px-2 py-1.5 text-left text-popover-foreground text-sm hover:bg-destructive hover:text-destructive-foreground"
